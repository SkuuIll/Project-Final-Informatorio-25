...
class PostViewTest(TestCase):
    ...
    def test_post_create_view_logged_out(self):
        ...
        self.assertRedirects(response, f"{reverse('accounts:login')}?next={reverse('posts:post_create')}")

    def test_post_update_view_author(self):
        """Prueba que el autor puede editar su propio post."""
        self.client.login(username='testuser', password='password')
        response = self.client.get(reverse('posts:post_update', kwargs={'slug': self.published_post.slug}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_like_post_view(self):
        ...
        self.client.login(username='testuser', password='password')
        response = self.client.post(reverse('posts:like_post', kwargs={'slug': self.published_post.slug}))
        self.assertEqual(response.status_code, 302) # Redirección
        self.published_post.refresh_from_db()
